...
Includes the 'traj_id' for stack and trajectories datasets.
Only selects events for which the isCC boolean variable is true.
...
with h5py.File("MiniRun4_1E19_RHC.flow.00000.FLOW.h5", "r") as file:
    ...
    trajID_stack = stack_dataset["traj_id"][:]
    ...
    trajID_trajectories = trajectories_dataset["traj_id"][:]
    ...
    for i in filtered_indices:
        ...
        csv_data.append({
            ...
            "trajID_stack": trajID_stack[i] if i < len(trajID_stack) else None,
            "trajID_trajectories": trajID_trajectories[i] if i < len(trajID_trajectories) else None
        })

    # Writing to CSV
    with open('eventID_vertexID_trajID_allMCdata_withBounds_CC.csv', 'w', newline='') as csvfile:
        fieldnames = ["Entry", "Pair (event_id, vertex_id)", "interactions", "stack", "segments", "trajectories", "trajID_stack", "trajID_trajectories"]
    ...
    for idx, i in enumerate(filtered_indices, start=1):
        ...
        for s_idx in range(stack_counts[i]):
            line += f"stack -> Entry {s_idx + 1}: ({eid}, {vid}), trajID_stack -> Entry {s_idx + 1}: {trajID_stack[s_idx]}, "
            ...
            if s_idx < trajectories_counts[i]:
                line += f"trajectories -> Entry {s_idx + 1}: ({eid}, {vid}), trajID_trajectories -> Entry {s_idx + 1}: {trajID_trajectories[s_idx]}"
            else:
                line += ","
        ...
        for t_idx in range(max(stack_counts[i], segments_counts[i]), trajectories_counts[i]):
            line += f"{' ' * 33}, , trajectories -> Entry {t_idx + 1}: ({eid}, {vid}), trajID_trajectories -> Entry {t_idx + 1}: {trajID_trajectories[t_idx]}"
            line += "\n"
        
        txt_data.append(line)

    # Writing to TXT
    with open('eventID_vertexID_trajID_allMCdata_withBounds_CC.txt', 'w') as txtfile:
        for line in txt_data:
            txtfile.write(line)

    # Writing to XLSX
    workbook = xlsxwriter.Workbook('eventID_vertexID_trajID_allMCdata_withBounds_CC.xlsx')
...
print("Summary saved to eventID_vertexID_trajID_allMCdata_withBounds_CC.csv, eventID_vertexID_trajID_allMCdata_withBounds_CC.xlsx, and eventID_vertexID_trajID_allMCdata_withBounds_CC.txt.")
