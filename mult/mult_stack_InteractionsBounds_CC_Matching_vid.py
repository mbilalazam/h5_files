...
for idx, filename in enumerate(filenames):
    with h5py.File(filename, 'r') as f:
        ...
        # Extract the isCC data, vertex data, and corresponding vertex_id from interactions dataset
        ...
        # Filter vertex_id values where isCC is true and within bounds
        cc_vertex_ids = vertex_id_interactions[isCC_mask & bounds_mask]
...
plt.xlabel('CC Multiplicity (Matching vertex_id within FV)')
plt.ylabel('Number of Events')
plt.title('Multiplicity Distribution (Minirun4_RHC)')
plt.xticks(range(1, 21))
plt.xlim(0.5, 20.5)
plt.legend(loc='upper right')
plt.savefig("mult_stack_InteractionsBounds_CC_Matching_vid.png", dpi=300)
plt.show()
